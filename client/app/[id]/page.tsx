...
import { useParams, useRouter } from 'next/navigation'; // <-- 1. Impor useRouter
...
export default function AssignmentPage() {
  ...
  // --- PERBAIKAN: Pindahkan state isSubmitting ke sini ---
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // --- PERBAIKAN: Logika validasi dan payload yang lebih baik ---
    if (assignment.type === 'pilgan') {
        ...
        alert('Harap isi jawaban esai Anda.');
        return;
    }

    if (window.confirm('Apakah Anda yakin ingin mengumpulkan jawaban Anda?')) {
        setIsSubmitting(true);
        // Siapkan payload berdasarkan tipe tugas
        ...
        try {
            ...
            // --- PERBAIKAN: Gunakan router untuk redirect ---
            router.push('/dashboard'); // Arahkan ke dashboard setelah berhasil
        } catch (error: any) {
            alert(error.response?.data?.message || 'Gagal mengumpulkan jawaban.');
        } finally {
  ...
  if (!assignment) return <div className="p-8 text-center">Kuis tidak ditemukan.</div>;
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
        ...
        {/* --- PERBAIKAN: Tampilkan soal berdasarkan tipe --- */}
        ...
        <div className="mt-8 pt-6 border-t flex justify-end">
          {/* --- PERBAIKAN: Hubungkan tombol ke state isSubmitting --- */}
          <button 
            type="submit" 
